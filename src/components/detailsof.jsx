...
import { CountContext } from "../context";
 function Detailsof  ({compo,num,setchats})  {
    ...
    const village=["Chinamiram","Anakoderu","rayalam"," Losarigutlapadu","Annavaram"]
...
//          Contexts.user({...Contexts.us,...value2})
  ...
  return (
    <div  className='selectplaces' >{array[compo].map((details)=>(<><div  className='selectplace' onClick={()=> detailsupdate(`${details}`)}>{details}</div></>))}</div>
  )
}
